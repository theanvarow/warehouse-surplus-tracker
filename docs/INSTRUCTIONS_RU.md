...
### Пошаговый алгоритм:
1. **Поле 1. Из какого короба вышел товар? (Номер короба)**  
   Отсканируйте штрих-код короба, который в данный момент пересчитывается (например, `KOROB-78421`).
...
5. **Поле 5. Куда переложен товар? (Итоговый короб):**  
   Отсканируйте штрих-код нового короба, в который складываются проблемные товары.
6. **Кнопка «Завершить»:**  
...
При сканировании товара выбирается одна из 8 утвержденных причин:
...
### Доступные метрики:
* **Зафиксировано сегодня товаров:** Общий объем зафиксированных единиц за текущую дату.
...
* **Топ причин:** Процентное соотношение причин брака и излишков.
* **Таблица записей:** Детальная лента фиксаций по каждому сотруднику и столу.
